...
%   Becasue the two GMM-HMMs take a lot of time to learn the model parameters and the
%   performance evaluation of the two take much more time than DNN and DNN-HMM, besides the
%   performance of the two is so bad, we omit the two GMM-HMMs and use only
%   DNN-HMM and related DNN here in enhancer identification.
%
%input:
...
%M: number of GMM components.   omit M 
%size:  structure of DNN
%
%output:
%PI:    initial state probability vecort of DNN-HMM
%A: state transition probability matrix of DNN-HMM.    always []
%B1:    all model parameters of Kmeans-GMM-HMM.   omit B1
%B3:    all model parameters of EM-GMM-HMM.   omit B3
...
T=[];
%% GMM  %测试时用了GMM，最终发现GMM效果太差，且耗时，所以注释掉GMM。不然费时不讨好，kmeans-GMM花费数百秒，EM-GMM花费一个小时，与后面DNN的耗时相当了。所以函数返回值去掉了B1和B3
