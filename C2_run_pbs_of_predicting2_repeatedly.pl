...
foreach my $test(@test_cells)
{
	foreach my $type(@types)
	{
		foreach my $training(@training_cells)
		{
			...
			{
				my $dir_out="$dir_out/$type.$training.20_50bp.$batch";
				mkdir $dir_out unless -e $dir_out;
				# foreach my $idx_partition(1..$num_partition)
				{
					{
						my $command="qsub -N $name -o $dir_out -e $dir_out -q $batch -l nodes=1:ppn=$ppn,mem=8gb -v type=\"\'$type\'\",training_cell=\"\'$training\'\",subdir=\"\'$subdir\'\",test_cell=\"\'$test\'\" $indir/C2_pbs_for_predicting2.sh";
						say $command;
						system($command);
					}
				}
