...
my $output="./Predict_result/Population_seq_for_Input_Data_chr/predict_result_of_Alu_region";
...
{
	my $input="./Predict_result/Population_seq_for_Input_Data_chr/predict_result_of_ERR$i.Alu.txt";
	if($i==188022){next;}

	open(f,$input)||die"error\n";
	while(<f>)
	{
		@a=split/\s+/,$_;
...
for(my$i=188021;$i<188026;$i++)
{
	my $Billy="/home/ouyang/project1-global_human/2-data-rnaEditing/merge-raw-data/AluMergeResults/ERR$i.Alu.vcf";
	if($i==188022){next;}
...
open(o,">./Predict_result/Population_seq_for_Input_Data_chr/overlap_of_Billy_and_DeepRed_merge_sample.Alu.txt")||die"error open overlap\n";
...
system("wc -l ./Predict_result/Population_seq_for_Input_Data_chr/overlap_of_Billy_and_DeepRed_merge_sample.Alu.txt");
