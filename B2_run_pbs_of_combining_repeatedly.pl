...
## 利用PBS调用MATLAB函数B2_combine_individual_classifier的参数
...
foreach my $type(@types)
{
	foreach my $cell(@cells)
	{
		foreach my $range(@ranges)
		{
			...
			{
				{
					my $command="qsub -o $dir_out -e $dir_out -q $batch -l nodes=1:ppn=$ppn,mem=10gb -v type=\"\'$type\'\",cell=\"\'$cell\'\",range=$range B2_pbs_for_combining.sh";
					say $command;
					system($command);
				}
			}
